...
# csmit -m 100G -c 32 "/applications/R/R-3.3.2/bin/Rscript permTest_MSH4_Rep1_rangerPeaks_vs_TEsRNAfams.R"
...
RNAplotDir <- "/home/ajt200/analysis/160902_Sasha_ChIP_MSH4_Rep1/peaks/PeakRanger1.18/ranger/p0.001_q0.01/MSH4_Rep1/genome_wide/regioneR/noMinWidth_mergedOverlaps/plots/TEsRNA/"
...
TEsRNAGR <- lapply(seq_along(RNAfamNames), function(x) {
  TEsRNA <- read.table(file = paste0(RNAdir,
...
save(ptPeaksTEsRNAPerChrom,
     file = paste0(outDir,
                   "permTest_MSH4_Rep1_rangerPeaks_vs_TEsRNA.RData"))
...
write.table(ptPeaksTEsRNAPerChromDataFrame,
            file = paste0(outDir,
                          "permTest_MSH4_Rep1_rangerPeaks_vs_TEsRNA_DataFrame.txt"),
            sep = "\t", row.names = F)

# plot graphical summaries of results
for(i in 1:length(ptPeaksTEsRNAPerChrom)) {
  pdf(file = paste0(RNAplotDir, RNAfamNames[i], "_permTest_nperm10000_MSH4_Rep1_rangerPeaks_perChrom.pdf"), width = 10, height = 7)
  plot(ptPeaksTEsRNAPerChrom[[i]], main = paste0("MSH4_Rep1_rangerPeaks vs ", RNAfamNames[i]), xlab = "Number of overlaps", ylab = "Density")
  dev.off()
  ...
  pdf(file = paste0(RNAplotDir, RNAfamNames[i], "_localZscore_permTest_nperm10000_MSH4_Rep1_rangerPeaks_w1kb_s50bp_w10kb_s500bp_w", win ,"kb_s", step, "bp_perChrom.pdf"))
  par(mar=c(5.1, 4.1, 4.1, 2.1))
  plot(lz_1kb, main = paste0("MSH4_Rep1_rangerPeaks vs ", RNAfamNames[i], " (1-kb shift)"))
  mtext(side = 3, at = 2, text = paste0("MSH4_Rep1_rangerPeaks vs ", RNAfamNames[i], " (1-kb shift)"))
  plot(lz_10kb, main = paste0("MSH4_Rep1_rangerPeaks vs ", RNAfamNames[i], " (10-kb shift)"))
  mtext(side = 3, at = 2, text = paste0("MSH4_Rep1_rangerPeaks vs ", RNAfamNames[i], " (10-kb shift)"))
  plot(lz_custom, main = paste0("MSH4_Rep1_rangerPeaks vs ", RNAfamNames[i], " (~", win, "-kb shift)"))
  mtext(side = 3, at = 2, text = paste0("MSH4_Rep1_rangerPeaks vs ", RNAfamNames[i], " (~", win, "-kb shift)"))
  dev.off()
}
