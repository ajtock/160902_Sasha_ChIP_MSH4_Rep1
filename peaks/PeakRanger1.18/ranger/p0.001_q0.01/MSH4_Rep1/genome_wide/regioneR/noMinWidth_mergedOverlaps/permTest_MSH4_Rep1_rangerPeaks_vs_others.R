...
# csmit -m 100G -c 48 "/applications/R/R-3.3.2/bin/Rscript permTest_MSH4_Rep1_rangerPeaks_vs_others.R"
...
# Well-positioned nucleosomes (ranger)
...
grl <- GRangesList("REC8_HA_Rep1GR" = REC8_HA_Rep1GR, "REC8_HA_Rep2GR" = REC8_HA_Rep2GR,
                   "REC8_MYC_Rep1GR" = REC8_MYC_Rep1GR, "kss_REC8_HA_Rep1GR" = kss_REC8_HA_Rep1GR,
...
save(ptPeaksOtherPerChrom,
     file = paste0(outDir,
                   "permTest_MSH4_Rep1_rangerPeaks_vs_others.RData"))
...
write.table(ptPeaksOtherPerChromDataFrame,
            file = paste0(outDir,
                          "permTest_MSH4_Rep1_rangerPeaks_vs_others_DataFrame.txt"),
            sep = "\t", row.names = F)

# plot graphical summaries of results
for(i in 1:length(ptPeaksOtherPerChrom)) {
  pdf(file = paste0(plotDir, otherNames[i], "_permTest_nperm10000_MSH4_Rep1_rangerPeaks_perChrom.pdf"), width = 10, height = 7)
  plot(ptPeaksOtherPerChrom[[i]], main = paste0("MSH4_Rep1_rangerPeaks vs ", otherNames[i]), xlab = "Number of overlaps", ylab = "Density")
  dev.off()
  ...
  pdf(file = paste0(plotDir, otherNames[i], "_localZscore_permTest_nperm10000_MSH4_Rep1_rangerPeaks_w1kb_s50bp_w10kb_s500bp_w", win ,"kb_s", step, "bp_perChrom.pdf"))
  par(mar=c(5.1, 4.1, 4.1, 2.1))
  plot(lz_1kb, main = paste0("MSH4_Rep1_rangerPeaks vs ", otherNames[i], " (1-kb shift)"))
  mtext(side = 3, at = 2, text = paste0("MSH4_Rep1_rangerPeaks vs ", otherNames[i], " (1-kb shift)"))
  plot(lz_10kb, main = paste0("MSH4_Rep1_rangerPeaks vs ", otherNames[i], " (10-kb shift)"))
  mtext(side = 3, at = 2, text = paste0("MSH4_Rep1_rangerPeaks vs ", otherNames[i], " (10-kb shift)"))
  plot(lz_custom, main = paste0("MSH4_Rep1_rangerPeaks vs ", otherNames[i], " (~", win, "-kb shift)"))
  mtext(side = 3, at = 2, text = paste0("MSH4_Rep1_rangerPeaks vs ", otherNames[i], " (~", win, "-kb shift)"))
  dev.off()
}
